# CLAUDE.md — Proyecto Vozdigital
# Este archivo es leido automaticamente por Claude Code al abrir el proyecto.
...
`CLAUDE.md` y `AGENTS.md` deben mantenerse sincronizados.
...
> **No es guia, es procedimiento.** Si saltás este pre-flight estás operando con
> contexto desactualizado y vas a generar drift documental. Origen del cambio:
...
La regla del Marco "si Claude detecta drift, debe corregirlo" es procedural, no
opcional. Decirle al usuario "encontré drift, voy a arreglarlo" y proceder.

---

## ⚠️ ENFORCEMENT AUTOMATICO — hooks de `.claude/settings.json`

Este proyecto tiene hooks reales que validan disciplina en cada `git push`.
**No los desactives ni los burles.**

| Hook | Trigger | Comportamiento | Override |
|------|---------|----------------|----------|
| `PreToolUse` (Bash) | comando matchea `git push.*origin <branch_protected>` | ejecuta `drift-audit.sh --strict`. Si detecta drift, exit 2 → push BLOQUEADO con mensaje de qué corregir. | agregar sufijo literal `--no-verify-marco` al comando, bajo responsabilidad explícita del operador |
| `Stop` | fin de sesión | ejecuta `drift-audit.sh --inform`. No bloquea, deja warning visible si hay drift. | — |

Branches protegidos por default: `main`, `master` (configurable en
`.claude/hooks/pre-push-check.sh` editando la variable `PROTECTED_BRANCHES`).

Los scripts viven en `.claude/hooks/`:
- `drift-audit.sh` — detector de drift (config-based, lee `.claude/marco.config.json`
  si el proyecto es multi-repo; degrada a single-repo si no existe el config)
- `pre-push-check.sh` — wrapper PreToolUse Bash
- `session-end-audit.sh` — wrapper Stop
...
del Marco Estructural (Paso 1-5 de CLAUDE.md maestro).
...
1. **Leer** `C:\Users\Dylhan\Desktop\SpecVSMarco\marco_estructural/CLAUDE.md` (protocolo de inicializacion y dispatch table)
...
### Resolución de la contradicción `Skill` tool vs CLAUDE.md (importante)

La tool `Skill` del harness Claude Code dice:

> "Only invoke a skill that appears in that list, or one the user explicitly typed as `/<name>` in their message."

Y este CLAUDE.md te dice "ejecutá checklists del Marco según la tarea". **Las dos políticas
parecen contradecirse.** En proyectos del Marco la regla operativa es:

1. **Si la skill aparece listada en el system-reminder de la sesión** (significa que el
   harness la registró como disponible), **podés invocarla proactivamente** cuando la
   dispatch table imperativa lo indique. La restricción "solo slash command" no aplica
   para skills del Marco listadas — el Marco es contexto pre-aprobado del proyecto.
2. **Si la skill NO aparece listada** (porque el harness no la cargó, o el agente NO es
   Claude Code), **leé el `.md` correspondiente directamente** desde
...
La diferencia es solo el mecanismo de invocación.
...
table imperativa de abajo lo indica. Pueden invocarse via Skill tool (si están listadas)
o leerse directamente como `.md`.
...
> hacé Y", hacelo antes de X o estás incumpliendo el Marco. La adopción de hooks
> (DEC-HOOKS-002) refuerza algunos de estos imperativos a nivel de enforcement
> automático en `git push` a branch protegido.
...
| Cambio arquitectónico (RLS, queue, integraciones) | invocar `debate` modo architect + registrar `decision` | aplicar `code-rules` | aplicar `pre-merge` |
| Cambios en auth, credenciales, secretos, encriptación | aplicar `security-review` ANTES también (no solo post) | `code-rules` + stack relevante | `security-review` + `pre-merge` |
...
> Si saltás estos, el push se rechaza por hook (cuando aplique) o vas a generar drift
> documental. Documentado por dolorosa experiencia (ver DEC-HOOKS-002).

1. **ANTES de `git push origin <branch_protected>`:**
   El hook `pre-push-check.sh` se ejecuta automáticamente. Si reporta drift, **arreglá
   el drift antes de pushear**, no uses `--no-verify-marco` salvo emergencia documentada.

2. **ANTES de declarar "fase cerrada" / "sub-fase cerrada":**
   Aplicar checklist `cierre-fase`. Eso incluye: actualizar `roadmap.md` + `STATUS.md`
   (si aplica) + cerrar cualquier TD marcada "prioritario para esta fase" + agregar
   entrada en `decisions.md` del **repo principal** (no solo en repos de componente
   si el proyecto es multi-repo).

3. **ANTES de tocar código de auth, secrets, permisos, RLS, encriptación:**
   Aplicar `security-review` ANTES (no después). El después también, pero el antes
   es el que evita decisiones malas.

4. **DESPUÉS de cerrar sprint o fase:**
   Aplicar `debate` modo reviewer. Es opcional para sub-fases; obligatorio para
   fases enteras y para hitos críticos (ej: cutover de migración).
...
Si se actualiza `CLAUDE.md`, reflejar el mismo cambio en `AGENTS.md`.
